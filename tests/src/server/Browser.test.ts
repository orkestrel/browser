 ...
 * Covers Browser lifecycle, factory, discovery, abort, and destroy semantics.
 */
...
describe('Browser', () => {
	describe('idle state', () => {
		...
		it('discover() returns well-shaped result', async () => {
	...
	describe('destroyed state', () => {
		...
		it('discover() still works after destroy (passive probe)', async () => {
			const browser = createBrowser({ cdp: { port: 19996 } })
			await browser.destroy()
			const result = await browser.discover()
			expect(result.found).toBe(false)
		})
	...
	describe('connected lifecycle', () => {
		it('connect() launches a real browser and transitions to connected', async () => {
			...
			// Can be 'launch' or 'cdp' depending on whether a CDP endpoint was discovered
			expect(['launch', 'cdp']).toContain(browser.connection)
			await browser.destroy()
		}, 30_000)
		...
		it('context(0) returns the same result as context()', async () => {
			const browser = createBrowser()
			await browser.connect()
			await browser.create()
		...
		it('page content can be extracted after navigation', async () => {
			...
			const page = await browser.create()
			await page.navigate(
				'data:text/html,<html lang="en"><head><title>Test</title></head><body><p>Content</p></body></html>',
			)
			const content = await page.content()
		...
		it('disconnect() detaches from the browser without closing it', async () => {
			const browser = createBrowser()
			await browser.connect()
			expect(browser.connected).toBe(true)
			browser.disconnect()
		...
		it('contexts() returns a new array each call', async () => {
			const browser = createBrowser()
			await browser.connect()
			await browser.create()
			const a = browser.contexts()
	...
	describe('external disconnect detection', () => {
		it('connected returns false when CDP client disconnects externally', async () => {
			const browser = createBrowser()
			await browser.connect()
			expect(browser.connected).toBe(true)

			// Simulate external disconnect by disconnecting then checking connected
			// The Browser's connected getter should detect the stale state
			browser.disconnect()
			expect(browser.connected).toBe(false)
			expect(browser.status).toBe('disconnected')
			await browser.destroy()
		...
		it('emits disconnect and idle events on external disconnect detection', async () => {
			...
			// Wait for construction idle
			await new Promise((resolve) => queueMicrotask(() => resolve(undefined)))
			const initialIdle = idleCount

			await browser.connect()
			browser.disconnect()

			expect(disconnectCount).toBeGreaterThan(0)
			expect(idleCount).toBeGreaterThan(initialIdle)
			await browser.destroy()
		}, 30_000)

		it('can reconnect after external disconnect', async () => {
			const browser = createBrowser()
			await browser.connect()
			expect(browser.connected).toBe(true)
			...
			// Reconnect should work from disconnected state
			await browser.connect()
			expect(browser.connected).toBe(true)
			await browser.destroy()
		}, 30_000)
	...
	describe('profile option', () => {
		it('connect() with profile launches with user data dir', async () => {
			const tmpDir = `${process.env['TEMP'] ?? process.env['TMPDIR'] ?? '/tmp'}/scsr-test-profile-${Date.now()}`
		...
		it('profile page can navigate', async () => {
			const tmpDir = `${process.env['TEMP'] ?? process.env['TMPDIR'] ?? '/tmp'}/scsr-test-profile-nav-${Date.now()}`
			const browser = createBrowser({ profile: tmpDir })
			await browser.connect()
			const page = await browser.create({ url: 'data:text/html,<h1>Profile</h1>' })
			expect(page.url).toContain('data:text/html')
			await browser.destroy()
		}, 30_000)
	})
...
describe('Browser Events', () => {
	...
	it('wires construction-time hooks via on option', () => {
		const browser = createBrowser({
			on: { discover: () => {} },
		})
	...
	it('emits idle event on disconnect', async () => {
		...
		// Wait for construction idle event
		await new Promise((resolve) => queueMicrotask(() => resolve(undefined)))
		const initialCount = idleCount
		await browser.connect()
		browser.disconnect()
		// Should have emitted idle again on disconnect
	...
	it('supports runtime on/off subscription', () => {
		const browser = createBrowser()
		const off = browser.emitter.on('disconnect', () => {})
	...
	it('emits page event on create', async () => {
		const browser = createBrowser()
		await browser.connect()
		let page: unknown
		browser.emitter.on('page', (p) => {
			page = p
		})
