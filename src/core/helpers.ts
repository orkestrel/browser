...
/**
 * Lazily walk every captured browser node in structural depth-first order.
 *
 * @remarks
 * Iframe content documents are entered at their owner element. Orphaned or
 * cyclic protocol nodes are still yielded once through an identity guard.
 *
 * @param snapshot - Captured browser snapshot
 * @returns A lazy node generator
 */
export function* walkBrowserSnapshot(
	snapshot: BrowserSnapshot,
): Generator<BrowserNode, void, unknown> {
	const seeds: BrowserNode[] = []
	for (const document of snapshot.documents) {
		seeds.push(...document.nodes.filter((node) => node.parent === undefined))
	}
	for (const document of snapshot.documents) seeds.push(...document.nodes)
	...
	for (const seed of seeds) {
		...
		while (stack.length > 0) {
			const node = stack.pop()
			if (node === undefined) break
			const identity = `${node.document}:${node.index}`
			if (visited.has(identity)) continue
			visited.add(identity)
			yield node

			const children = childrenOfBrowserNode(snapshot, node)
			for (let index = children.length - 1; index >= 0; index -= 1) {
				const child = children[index]
...
/**
 * Lazily walk every captured node in structural breadth-first order.
 *
 * @param snapshot - Captured browser snapshot
 * @returns A lazy breadth-first generator
 */
export function* walkBrowserSnapshotBreadthFirst(
	snapshot: BrowserSnapshot,
): Generator<BrowserNode, void, unknown> {
	const seeds: BrowserNode[] = []
	for (const document of snapshot.documents) {
	...
	for (const seed of seeds) {
		...
		while (cursor < queue.length) {
			...
			visited.add(identity)
			yield node
			queue.push(...childrenOfBrowserNode(snapshot, node))
		}
	}
}

/**
 * Lazily walk one captured browser-node subtree in depth-first order.
 *
 * @param snapshot - Captured browser snapshot
 * @param root - Root node, included as the first result
 * @returns A lazy subtree generator
 */
export function* walkBrowserNode(
	snapshot: BrowserSnapshot,
	root: BrowserNode,
): Generator<BrowserNode, void, unknown> {
...
/**
 * Lazily walk one captured browser-node subtree in breadth-first order.
 *
 * @param snapshot - Captured browser snapshot
 * @param root - Root node, included as the first result
 * @returns A lazy breadth-first subtree generator
 */
export function* walkBrowserNodeBreadthFirst(
	snapshot: BrowserSnapshot,
	root: BrowserNode,
): Generator<BrowserNode, void, unknown> {
	const queue: BrowserNode[] = [root]
	const visited = new Set<string>()
	let cursor = 0
	while (cursor < queue.length) {
		const node = queue[cursor]
...
/**
 * Lazily walk descendants of one captured node, excluding the node itself.
 *
 * @param snapshot - Captured browser snapshot
 * @param root - Root whose descendants are required
 * @returns A lazy descendant generator
 */
export function* descendantsOfBrowserNode(
	snapshot: BrowserSnapshot,
	root: BrowserNode,
): Generator<BrowserNode, void, unknown> {
	let first = true
	for (const node of walkBrowserNode(snapshot, root)) {
		if (first) {
			first = false
			continue
		}
...
/**
 * Resolve the document containing a browser node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Node whose document is required
 * @returns The containing document, or undefined
 */
export function documentOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
): BrowserDocument | undefined {
	return snapshot.documents.find((document) => document.index === node.document)
}

/**
 * Return direct children of a captured browser node.
 *
 * @remarks
 * Includes the root nodes of an iframe's linked content document. Shadow DOM
 * and template content already carry flattened parent indexes from CDP.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Parent node
 * @returns Direct children in snapshot order
 */
export function childrenOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
): readonly BrowserNode[] {
	const document = documentOfBrowserNode(snapshot, node)
	if (document === undefined) return []
	const children = document.nodes.filter((candidate) => candidate.parent === node.index)
	if (node.content === undefined) return children
	const content = snapshot.documents.find((candidate) => candidate.index === node.content)
	if (content === undefined) return children
	return [...children, ...content.nodes.filter((candidate) => candidate.parent === undefined)]
}

/**
 * Resolve the structural parent of a captured browser node.
 *
 * @remarks
 * An iframe content-document root resolves to its owning iframe node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Child node
 * @returns Parent node, or undefined for a top-level document root
 */
export function parentOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
	...
	return node.parent === undefined
		? findBrowserNode(snapshot, (candidate) => candidate.content === node.document)
		: document.nodes[node.parent]
}

/**
 * Return every structural sibling except the supplied browser node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Node whose siblings are required
 * @returns Siblings in structural order
 */
export function siblingsOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
): readonly BrowserNode[] {
	const parent = parentOfBrowserNode(snapshot, node)
	const candidates =
		parent === undefined
			? (documentOfBrowserNode(snapshot, node)?.nodes.filter(
					(candidate) => candidate.parent === undefined,
				) ?? [])
			: childrenOfBrowserNode(snapshot, parent)
...
/**
 * Return structural siblings before the supplied browser node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Reference node
 * @returns Earlier siblings in structural order
 */
export function precedingSiblingsOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
): readonly BrowserNode[] {
	const parent = parentOfBrowserNode(snapshot, node)
	...
	return index < 0 ? [] : candidates.slice(0, index)
}

/**
 * Return structural siblings after the supplied browser node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Reference node
 * @returns Later siblings in structural order
 */
export function followingSiblingsOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
): readonly BrowserNode[] {
	const parent = parentOfBrowserNode(snapshot, node)
	...
	return index < 0 ? [] : candidates.slice(index + 1)
}

/**
 * Return the nearest-first ancestor chain for a captured browser node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Descendant node
 * @returns Nearest-first ancestor list
 */
export function ancestorsOfBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
): readonly BrowserNode[] {
	...
	while (true) {
		const parent = parentOfBrowserNode(snapshot, current)
		if (parent === undefined) break
		const identity = `${parent.document}:${parent.index}`
...
/**
 * Test whether one captured node is below another across frame boundaries.
 *
 * @param snapshot - Captured browser snapshot
 ...
 */
export function isBrowserNodeDescendant(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
	ancestor: BrowserNode,
): boolean {
	return ancestorsOfBrowserNode(snapshot, node).some(
		(candidate) => candidate.document === ancestor.document && candidate.index === ancestor.index,
	)
}

/**
 * Resolve the nearest common ancestor of two captured browser nodes.
 *
 * @param snapshot - Captured browser snapshot
 ...
 */
export function commonAncestorOfBrowserNodes(
	snapshot: BrowserSnapshot,
	first: BrowserNode,
	second: BrowserNode,
): BrowserNode | undefined {
	const firstAncestors = [first, ...ancestorsOfBrowserNode(snapshot, first)]
	const secondAncestors = [second, ...ancestorsOfBrowserNode(snapshot, second)]
	const identities = new Set(firstAncestors.map((node) => `${node.document}:${node.index}`))
	return secondAncestors.find((node) => identities.has(`${node.document}:${node.index}`))
}

/**
 * Compute the edge count between two captured browser nodes.
 *
 * @param snapshot - Captured browser snapshot
 * @param first - First node
 * @param second - Second node
 * @returns Tree distance, or undefined when no common ancestor exists
 */
export function computeBrowserNodeDistance(
	snapshot: BrowserSnapshot,
	first: BrowserNode,
	second: BrowserNode,
): number | undefined {
	const common = commonAncestorOfBrowserNodes(snapshot, first, second)
	if (common === undefined) return undefined
	const firstPath = [first, ...ancestorsOfBrowserNode(snapshot, first)]
	const secondPath = [second, ...ancestorsOfBrowserNode(snapshot, second)]
	const firstDistance = firstPath.findIndex(
		(node) => node.document === common.document && node.index === common.index,
	)
	const secondDistance = secondPath.findIndex(
		(node) => node.document === common.document && node.index === common.index,
	)
...
/**
 * Find the first captured node satisfying a predicate.
 *
 * @param snapshot - Captured browser snapshot
 * @param predicate - Node predicate
 * @returns First matching node, or undefined
 */
export function findBrowserNode(
	snapshot: BrowserSnapshot,
	predicate: BrowserNodePredicate,
): BrowserNode | undefined {
	for (const node of walkBrowserSnapshot(snapshot)) {
...
/**
 * Find captured nodes satisfying a predicate.
 *
 * @param snapshot - Captured browser snapshot
 * @param predicate - Node predicate
 * @param limit - Optional maximum result count
 * @returns Matching nodes in snapshot order
 */
export function findBrowserNodes(
	snapshot: BrowserSnapshot,
	predicate: BrowserNodePredicate,
	limit?: number,
): readonly BrowserNode[] {
	if (limit !== undefined && (!isInteger(limit) || limit < 0)) {
		throw new BrowserError('Browser node result limit must be a non-negative integer', undefined, {
			limit,
		})
	}
	if (limit === 0) return []
...
/**
 * Find the first matching descendant beneath one captured node.
 *
 * @param snapshot - Captured browser snapshot
 * @param root - Root whose descendants are searched
 * @param predicate - Node predicate
 * @returns First matching descendant, or undefined
 */
export function findBrowserDescendant(
	snapshot: BrowserSnapshot,
	root: BrowserNode,
	predicate: BrowserNodePredicate,
): BrowserNode | undefined {
	for (const node of descendantsOfBrowserNode(snapshot, root)) {
		if (predicate(node)) return node
	}
	return undefined
}
...
/**
 * Find the nearest matching node from the candidate through its ancestors.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Starting node, included in the search
 * @param predicate - Node predicate
 * @returns Closest matching node, or undefined
 */
export function closestBrowserNode(
	snapshot: BrowserSnapshot,
	node: BrowserNode,
	predicate: BrowserNodePredicate,
): BrowserNode | undefined {
	if (predicate(node)) return node
	return ancestorsOfBrowserNode(snapshot, node).find(predicate)
}
...
/**
 * Build a deterministic, serializable path for a captured browser node.
 *
 * @param snapshot - Captured browser snapshot
 * @param node - Captured browser node
 * @returns Frame-qualified structural path
 */
export function nodeToPath(snapshot: BrowserSnapshot, node: BrowserNode): string {
	const document = documentOfBrowserNode(snapshot, node)
	if (document === undefined) return `document:${node.document} > node:${node.index}`
	const chain = [...ancestorsOfBrowserNode(snapshot, node)].reverse()
	chain.push(node)
	...
	for (const current of chain) {
		...
		if (current.parent !== undefined) {
			const owner = documentOfBrowserNode(snapshot, current)
