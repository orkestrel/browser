...
| `launchBrowserProcess`    | function | Launch a browser process with raw-CDP debugging flags, isolated in its own POSIX process group for complete teardown; returns the spawned `ChildProcess`.                                                                                                                                                                                                      |
...
12. **`Browser.destroy()` escalates SIGTERM → SIGKILL; `close()` is graceful.**
    ...
    `destroy()` sends the launched process tree `SIGTERM`; if the parent has
    not exited or the group has not drained after `BROWSER_KILL_GRACE_MS`, the
    tree is force-killed with `SIGKILL` and given the same bounded exit window.
